...
SCHLUESSEL = {
    ...
    "1": ["a", False],  # Agree
    "2": ["b", False],  # Agree slightly
    "3": ["d", False],  # Disagree
    "4": ["e", False],  # Disagree slightly
    "5": ["a", True],  # Agree (Priorisiert)
    "6": ["b", True],  # Agree slightly(Priorisiert)
    "7": ["d", True],  # Disagree (Priorisiert)
    "8": ["e", True],  # Disagree slightly (Priorisiert)
    "9": ["n", True],  # Neutral (Priorisiert)
    "A": ["n", False],  # Neutral
    "B": ["s", False],  # These übersprungen (Skip)
...
def encode_zustand(opinions):
    ...
    for these, s in opinions:
        ...
        zustand += int(status, 36) * (36**these.pk)
...
def decode_zustand(zustand=0):
    ...
    old_list = list(numpy.base_repr(zustand, 36))[::-1]

    for pk, status in enumerate(old_list):
